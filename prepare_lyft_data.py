...
def prepare_frustum_data_from_scenes(num_entries_to_get: int,
    ...
    save_file(output_filename, token_filename, annotation_token_list, box2d_list, box3d_list, box3d_size_list,
              camera_data_token_list, from_rgb_detection, frustum_angle_list, heading_list, id_list, input_list,
              label_list, prob_list, sample_token_list, type_list)


def save_file(output_filename, token_filename, annotation_token_list, box2d_list, box3d_list, box3d_size_list,
              camera_data_token_list, from_rgb_detection, frustum_angle_list, heading_list, id_list, input_list,
              label_list, prob_list, sample_token_list, type_list):
    # check that everything is implemented
    if not from_rgb_detection:
...
def prepare_frustum_data_from_selected_scenes(restore_progress=True):
    output_file_base = os.path.join(ARTIFACT_PATH,"lyft_val_3_{}.pickle")
    token_file_base = os.path.join(ARTIFACT_PATH,"lyft_val_token_{}.pickle")

    # total training scenes: 180
    batch_scene_num = 5  # number of batches required in a scene
    end_scene_num = 100

    start_scene_num = 0
    saver_file = "prepare_frustum_progress.pickle"
    if restore_progress and os.path.exists(saver_file):
        with open(saver_file, 'rb') as fp:
            start_scene_num = pickle.load(fp)

    for start_scene in range(start_scene_num, end_scene_num, batch_scene_num):
        with open(saver_file, 'wb') as fp:
            pickle.dump(start_scene,fp)
        working_scenes = np.arange(start_scene, start_scene + batch_scene_num)
        print("working_scenes:", working_scenes)
        output_file = output_file_base.format(start_scene)
        token_file = output_file_base.format(start_scene)
        prepare_frustum_data_from_scenes(5120000, output_file, lyftdf=level5data, token_filename=token_file,
...
if __name__ == "__main__":
    ...
    prepare_frustum_data_from_selected_scenes(restore_progress=True)
